...
### Session-long attention camera (2026-07-20)
Changed the parent attention camera from opt-in per-child to on for the whole
login (login → logout). Client decisions: consent asked once per login; camera
runs from login even before a child is picked; manual "Stop camera" and the
3-min minimum removed. Approach A — replaced `AttentionPanel` with a
`SessionCamera` component always mounted at the top of the parent portal (which
unmounts on logout, so "mounted" == the login). `useAttention` runs continuously;
attention is attributed as per-session deltas: on each `activeSessionId` change,
and on logout/unmount, the delta accrued since the last boundary is POSTed to the
session that was active, then rebased. Backend, DB schema, tracker/accumulator and
the engagement scorer are untouched; time with no child selected isn't recorded.
Design doc: `2026-07-20-session-long-camera-design.md`. `tsc --noEmit` clean.
Not yet verified in-browser (needs the dev servers + a real webcam / the
`__ATTENTION_MOCK__` path).

### Attention history reliability fix (2026-07-20, follow-up)
Found a real bug in the session-long camera: the final flush on logout runs at
component unmount, but logout (`signOut` in PortalLayout) clears the Supabase
token first, so `authHeaders()` throws and the POST never sends — the child being
observed at logout (the common case) was never saved. Fix: (1) backend
`record_attention` now upserts one `attention_scores` row per session instead of
always inserting (no unique constraint on session_id, so select-then-update/insert);
this also keeps the engagement scorer's per-session average correct. (2) SessionCamera
saves the current session's running total every 15s, so all but the last few seconds
persist before logout regardless of the token race. (3) Parent dashboard auto-refreshes
the Attention History tab every 15s while it's open. tsc + backend parse clean.

### Focus mode — in-browser attention lock (2026-07-21)
Parent request: stop parents leaving to other tabs/sites while monitoring. Told them
a website CANNOT truly block tab-switching (browser security) — built detect+deter
instead. New `useFocusGuard` hook (visibilitychange + window blur/focus, 5s grace,
`suppress()` for intentional new-tab opens like report-card downloads) → full-screen
`FocusGuardOverlay` "Monitoring paused" blocking the portal on return. Each leave logged
to the session: new `focus_losses` + `away_seconds` columns on monitoring_sessions
(migration 2026-07-21-focus-mode.sql), written via `POST /parent/sessions/{id}/focus-loss`,
